 ...
 */
public class ECEliteEncoderRecipeTransferHandler implements IRecipeTransferHandler<ContainerExtremePatternEncoder> {
    @Override
    ...
    public IRecipeTransferError transferRecipe(ContainerExtremePatternEncoder container, IRecipeLayout recipeLayout, EntityPlayer player, boolean maxTransfer, boolean doTransfer) {
        if (doTransfer) {
            ...
            InventoryCrafting inventory = new InventoryCrafting(container, 7, 7);
            NonNullList<ItemStack> items = NonNullList.withSize(7 * 7, ItemStack.EMPTY);
            ...
            container.getTile().setTableSize(ExtendedCraftingUtil.TableSize.ELITE);
